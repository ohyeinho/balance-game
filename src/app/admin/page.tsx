...
function AdminGroupView({
  ...
  code: string;
  groupResponses: GameResponse[];
  handleDeleteOne: (id: string, name: string) => void;
  handleResetCode: (code: string) => void;
}) {
  const getRankedCouples = (order: "desc" | "asc") => {
    if (groupResponses.length < 2) return [];
    ...
    for (let i = 0; i < groupResponses.length; i++) {
      for (let j = i + 1; j < groupResponses.length; j++) {
        ...
        for (let q = 1; q <= TOTAL_QUESTIONS; q++) {
          if (
            groupResponses[i].answers[String(q)] ===
            groupResponses[j].answers[String(q)]
          ) {
            matches++;
          }
        }
        pairs.push({
          nameA: groupResponses[i].name,
          nameB: groupResponses[j].name,
          matches,
        });
      }
    }
    pairs.sort((a, b) =>
      order === "desc" ? b.matches - a.matches : a.matches - b.matches
    );
    ...
    pairs.forEach((pair) => {
      if (
        ranks.length === 0 ||
        ranks[ranks.length - 1].matches !== pair.matches
      ) {
  ...
  return (
    <div className="bg-transparent h-full pb-8">
      {/* Best Matching Pairs */}
      {topRanks.length > 0 && (
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-5 border border-gray-100 mb-6 shadow-sm">
          <h2 className="text-base font-extrabold text-gray-800 mb-1 text-center">
            Best Matching Pairs ({code})
          </h2>
            ...
            {topRanks.map((rank, rankIdx) =>
              rank.couples.map((couple, cIdx) => (
                ...
                >
                  ...
                  <div
                    className={`text-white text-xs font-extrabold px-3 py-1.5 rounded-full bg-gradient-to-r ${
                      medalColors[rankIdx] || "from-gray-400 to-gray-500"
                    }`}
                  >
            ...
            {bottomRanks.map((rank, rankIdx) =>
              rank.couples.map((couple, cIdx) => (
                ...
                >
                  ...
                  <div
                    className={`text-white text-xs font-extrabold px-3 py-1.5 rounded-full bg-gradient-to-r ${
                      worstColors[rankIdx] || "from-gray-400 to-gray-500"
                    }`}
                  >
                    {Math.round((couple.matches / TOTAL_QUESTIONS) * 100)}%
                  </div>
...
export default function AdminPage() {
  ...
  const handleResetCode = async (code: string) => {
    if (
      !window.confirm(
        `정말로 입장코드 [${code}]의 모든 응답 데이터를 초기화하시겠습니까?\n이 작업은 되돌릴 수 없습니다.`
      )
    ) {
      return;
    }
    if (!window.confirm(`마지막 확인입니다. 정말 [${code}] 데이터를 삭제하시겠습니까?`)) {
      return;
    }
    ...
    try {
      const res = await fetch("/api/results", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ password, entryCode: code }),
      });

      if (res.ok) {
        setResponses((prev) => 
          prev.filter((r) => (r.entryCode || "100830") !== code)
        );
        alert(`입장코드 [${code}] 데이터가 초기화되었습니다.`);
      } else {
        alert("초기화에 실패했습니다.");
      }
    } catch {
    ...
    <div className="min-h-screen px-4 py-6 animate-fade-in flex flex-col">
      ...
      >
        {codes.map((code) => {
          ...
          return (
            <div key={code} id={`slide-${code}`} className="w-full shrink-0 snap-center px-0">
              <AdminGroupView 
                code={code} 
